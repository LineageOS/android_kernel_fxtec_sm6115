...
* aw87xxx marco
******************************************************************/
#define AW87XXX_I2C_NAME1		"aw87xxx_pa1"
#define AW87XXX_DRIVER_VERSION1		"v0.2.0"
#define AW87XXX_PRODUCT_NUM1		4
#define AW87XXX_PRODUCT_NAME_LEN1	8
...
static char aw87xxx_support_product[][AW87XXX_PRODUCT_NAME_LEN1] = {
	{"aw87339"},
	{"aw87359"},
	{"aw87369"},
	{"aw87519"},
...
unsigned char aw87xxx_show_current_mode1(int32_t channel)
{
...
EXPORT_SYMBOL(aw87xxx_show_current_mode1);

int aw87xxx_audio_scene_load1(uint8_t mode, int32_t channel)
{
	struct aw87xxx_scene_param *scene_param;
	struct list_head *pos;
	struct aw87xxx *aw87xxx;
	...
	list_for_each(pos, &g_aw87xxx_device_list) {
		...
		if (aw87xxx->pa_channel == channel) {
			...
			if (aw87xxx->aw87xxx_scene_ls.scene_off.cfg_update_flag != AW87XXX_CFG_OK) {
...
EXPORT_SYMBOL(aw87xxx_audio_scene_load1);
				    ...
				    const struct firmware *cont)
{
	...
	if (check_sum != sum_data) {
		aw_dev_dbg(aw87xxx->dev, "%s: parse non head firmware!\n",
			__func__);
		return AW87XXX_OLD_FIRMWARE;
	} else {
		aw_dev_dbg(aw87xxx->dev, "%s: parse add head firmware!\n",
			__func__);
...
static void aw87xxx_vmax_cfg_loaded(const struct firmware *cont, void *context)
{
	...
	vmax_cfg_len = (vmax_cfg_num * sizeof(struct vmax_single_config) +
			sizeof(vmax_cfg_num));
	vmax_cfg = aw87xxx->monitor.vmax_cfg =
	    devm_kzalloc(aw87xxx->dev, vmax_cfg_len, GFP_KERNEL);
	if (!vmax_cfg) {
		aw_dev_err(aw87xxx->dev, "%s: vmax_cfg devm_kzalloc failed\n",
			__func__);
		return;
	}
	aw_dev_info(aw87xxx->dev, "%s: vmax_cfg_num=[%d],alloc mem=[%d]",
		__func__, vmax_cfg_num, vmax_cfg_len);
...
static int aw87xxx_product_select(struct aw87xxx *aw87xxx, uint8_t chip_type)
{
	...
	case AW87XXX_339:
		...
		aw87xxx->reg_param.reg_access = aw87339_reg_access1;
		break;
	case AW87XXX_359:
		...
		aw87xxx->reg_param.reg_access = aw87359_reg_access1;
		break;
	case AW87XXX_369:
		...
		aw87xxx->reg_param.reg_access = aw87369_reg_access1;
		break;
	case AW87XXX_519:
		...
		aw87xxx->reg_param.reg_access = aw87519_reg_access1;
		break;
	case AW87XXX_559:
		...
		aw87xxx->reg_param.reg_access = AW87559_reg_access1;
		break;
	default:
				...
				size_t len)
{
	...
	aw_dev_dbg(aw87xxx->dev, "%s: set hardware enable: %d\n", __func__,
		state);
	if (state == 0)
		aw87xxx_hw_off(aw87xxx);
	else
				  ...
				  struct device_node *np)
{
	...
	if (aw87xxx->reset_gpio < 0) {
		...
		aw_dev_info(aw87xxx->dev, "%s: reset gpio provided ok %d\n",
...
static void aw87xxx_parse_dt(struct aw87xxx *aw87xxx, struct device_node *np)
{
	...
	aw87xxx_parse_monitor_dt1(&aw87xxx->monitor);
...
static int aw87xxx_update_cfg_name(struct aw87xxx *aw87xxx)
{
	...
	aw_dev_info(aw87xxx->dev, "%s: chipid: %d %s %s\n",
						__func__, aw87xxx->chipid,aw87xxx->channel,buf);

	for (i = 0; i < sizeof(aw87xxx->cfg_name) / AW87XXX_CFG_NAME_MAX; i++)
	{
		memcpy(aw87xxx->cfg_name[i] + head_index, buf, sizeof(buf) - 1);
		aw_dev_info(aw87xxx->dev, "%s: chipid: %s\n",
	...
	for (i = 0; i < sizeof(aw87xxx->cfg_name) / AW87XXX_CFG_NAME_MAX; i++) {
		if (aw87xxx->channel[0] != '-') {
			...
			aw_dev_info(aw87xxx->dev, "%s: cfgnametpr: %s\n",
								__func__, aw87xxx->cfg_name[i]);
			cfg_size = strlen(aw87xxx->cfg_name[i]);
			memcpy(aw87xxx->cfg_name[i] + cfg_size, aw87xxx_end,
			       strlen(aw87xxx_end));
			aw_dev_info(aw87xxx->dev, "%s: cfg_name:11 %s\n",
				    __func__, aw87xxx->cfg_name[i]);
		} else {
			cfg_size = strlen(aw87xxx->cfg_name[i]);
			memcpy(aw87xxx->cfg_name[i] + cfg_size - 1, aw87xxx_end,
			       strlen(aw87xxx_end));
			aw_dev_info(aw87xxx->dev, "%s: cfg_name:22 %s\n",
				    __func__, aw87xxx->cfg_name[i]);
		}
	}
	...
	if (aw87xxx->channel[0] != '-') {
		...
		memcpy(aw87xxx->vmax_cfg_name + head_index, buf, sizeof(buf) - 1);
		memcpy(aw87xxx->vmax_cfg_name + strlen(aw87xxx->vmax_cfg_name),
		       aw87xxx->channel, strlen(aw87xxx->channel));
		memcpy(aw87xxx->vmax_cfg_name + strlen(aw87xxx->vmax_cfg_name),
		       aw87xxx_end, strlen(aw87xxx_end));
		aw_dev_info(aw87xxx->dev,
			    "%s: dev_i2c%d@0x%02X, vmax_name:33 %s\n", __func__,
			    aw87xxx->i2c_seq, aw87xxx->i2c_addr,
			    aw87xxx->vmax_cfg_name);
	} else {
		memcpy(aw87xxx->vmax_cfg_name, aw87xxx_vmax_cfg_name,
		...
		memcpy(aw87xxx->vmax_cfg_name + strlen(aw87xxx->vmax_cfg_name) - 1,
		       aw87xxx_end, strlen(aw87xxx_end));
		aw_dev_info(aw87xxx->dev,
			    "%s: dev_i2c%d@0x%02X, vmax_name:44 %s\n", __func__,
			    aw87xxx->i2c_seq, aw87xxx->i2c_addr,
			    aw87xxx->vmax_cfg_name);
	}
			     ...
			     const struct i2c_device_id *id)
{
	...
	aw87xxx_hw_off(aw87xxx);
	aw87xxx_monitor_init1(&aw87xxx->monitor);
	return 0;
...
static struct i2c_driver aw87xxx_i2c_driver1 = {
	.driver = {
		   .owner = THIS_MODULE,
		   .name = AW87XXX_I2C_NAME1,
...
static int __init aw87xxx_pa_init1(void)
{
	int ret;

	pr_info("%s enter, driver version: %s\n", __func__,
		AW87XXX_DRIVER_VERSION1);
	ret = i2c_add_driver(&aw87xxx_i2c_driver1);
	if (ret) {
...
module_init(aw87xxx_pa_init1);

static void __exit aw87xxx_pa_exit1(void)
{
	pr_info("%s enter\n", __func__);
	i2c_del_driver(&aw87xxx_i2c_driver1);
}

module_exit(aw87xxx_pa_exit1);
