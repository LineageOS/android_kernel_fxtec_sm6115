...
int aw_send_afe_module_enable1(void *buf, int size, uint8_t type)
{
	int ret;
...
int aw_get_dsp_msg_data1(char *data_ptr, int data_size, int inline_id,
			int channel)
{
	int32_t cmd_msg[6] = { 0 };
	int ret;
...
EXPORT_SYMBOL(aw_get_dsp_msg_data1);
...
*****************************************************/
void aw87xxx_monitor_stop1(struct aw87xxx_monitor *monitor)
{
	...
	aw_dev_info(aw87xxx->dev, "%s enter, dev_i2c%d@0x%02X\n", __func__,
		aw87xxx->i2c_seq, aw87xxx->i2c_addr);
	cancel_delayed_work_sync(&aw87xxx->monitor.work);

}
EXPORT_SYMBOL(aw87xxx_monitor_stop1);
...
void aw87xxx_monitor_work_func1(struct work_struct *work)
{
	int ret;
	uint32_t vbat_capacity;
...
EXPORT_SYMBOL(aw87xxx_monitor_work_func1);
...
***********************************************************/
void aw87xxx_monitor_init1(struct aw87xxx_monitor *monitor)
{
	...
	INIT_DELAYED_WORK(&monitor->work, aw87xxx_monitor_work_func1);
...
EXPORT_SYMBOL(aw87xxx_monitor_init1);

void aw87xxx_parse_monitor_dt1(struct aw87xxx_monitor *monitor)
{
	int ret;
	struct aw87xxx *aw87xxx = container_of(monitor,
		struct aw87xxx, monitor);
...
EXPORT_SYMBOL(aw87xxx_parse_monitor_dt1);
