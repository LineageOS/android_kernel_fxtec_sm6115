...
int mas_sync(u8 *txb, u8 *rxb, int len) {
    int ret = 0;
#if 0    //kingsun/zlc:  fixed the iuuse which the fingerprint menu disappear from setting.
    mutex_lock(&dev_lock);
...
static long mas_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
    switch(cmd){
        ...
        case MA_IOC_GVER:
	        ...
	        ret = copy_to_user((unsigned int *)arg, &MA_DRV_VERSION, sizeof(unsigned int));
            mutex_unlock(&ioctl_lock);
            break;
		case SCREEN_ON:
