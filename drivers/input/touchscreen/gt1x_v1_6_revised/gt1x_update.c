...
gt1x_update_pre_fail1:
	if (update_info.update_type == UPDATE_TYPE_REQUEST) {
		release_firmware(update_info.fw);
		update_info.fw = NULL;
	} else if (update_info.update_type == UPDATE_TYPE_FILE) {
...
void gt1x_update_cleanup(void)
{
	if (update_info.update_type == UPDATE_TYPE_FILE) {
		...
		set_fs(update_info.old_fs);
	} else if (update_info.update_type == UPDATE_TYPE_REQUEST) {
		if (update_info.fw) {
			release_firmware(update_info.fw);
			update_info.fw = NULL;
		}
...
int gt1x_check_firmware(void)
{
	...
	GTP_INFO("Update type: %s", update_info.update_type == UPDATE_TYPE_REQUEST ? "RequestFW" : "FileFW");
