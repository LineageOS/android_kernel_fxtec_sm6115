		...
		size_t size, loff_t * ppos)
{
	...
	temp_buff = (char *)kmalloc(GTP_CONFIG_MAX_LENGTH * 2 + GTP_CONFIG_EXT_LENGTH * 2 + 350, GFP_KERNEL);
	if (!temp_buff) {
...
static s32 gt1x_enter_sleep(void)
{
...
#else
	{
		...
		if (gt1x_wakeup_level == 1) {
			...
			if ( gt_pinctrl->ts_pinctrl && gt_pinctrl->pinctrl_sleep)
				ret = pinctrl_select_state(gt_pinctrl->ts_pinctrl, gt_pinctrl->pinctrl_sleep);
			if (ret < 0) 
...
void gt1x_power_reset(void)
{
	...
	gt1x_irq_disable();
	gt1x_power_switch(SWITCH_OFF);
      gt1x_vcc_i2c_switch(SWITCH_OFF);
	msleep(30);
	gt1x_power_switch(SWITCH_ON);
	 gt1x_vcc_i2c_switch(SWITCH_ON);
		...
		struct input_dev * pen_dev)
{
	...
	GTP_DEBUG("finger num %d",touch_num);
