...
{
    aw_dev_info(aw8622x->dev, "%s aw8622x->gain=0x%x gain=0x%x\n",__func__, aw8622x->gain,gain);
...
static int aw8622x_haptic_cont_get_f0(struct aw8622x *aw8622x)
{
	...
	if (get_f0_flag) {
		ret = aw8622x_haptic_read_lra_f0(aw8622x);
...
static int aw8622x_haptic_ram_config(struct aw8622x *aw8622x, int duration)
{
    ...
    aw_dev_info(aw8622x->dev, "%s: duration:[%d] time_list = [%d %d %d]\n", __func__, duration,aw8622x->dts_info.duration_time[0],aw8622x->dts_info.duration_time[1],aw8622x->dts_info.duration_time[2]);
				  ...
				  const char *buf, size_t count)
{
	...
	if (val) {
		mutex_lock(&aw8622x->lock);
		aw8622x_haptic_upload_lra(aw8622x, WRITE_ZERO);
		rc = aw8622x_haptic_f0_calibration(aw8622x);
        if(rc < 0)
        {
		    count = rc;
	    }
...
static void aw8622x_long_vibrate_work_routine(struct work_struct *work)
{
	...
	aw_dev_info(aw8622x->dev, "%s enter aw8622x->state=%d aw8622x->activate_mode=%d\n", __func__,aw8622x->state,aw8622x->activate_mode);
	...
	if (aw8622x->state) {
		...
		/* run ms timer */
        aw_dev_info(aw8622x->dev, "%s set timer :%d s : %dms\n", __func__,aw8622x->duration / 1000,(aw8622x->duration % 1000) * 1000000);
